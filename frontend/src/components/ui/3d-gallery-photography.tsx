...
function GalleryScene({
    ...
    const spatialPositions = useMemo(() => {
        ...
        const maxH = MAX_HORIZONTAL_OFFSET;
        const maxV = MAX_VERTICAL_OFFSET;

        // Seeded pseudo-random for deterministic but scattered positions
        ...
        for (let i = 0; i < visibleCount; i++) {
            // Generate random-looking positions using different seeds per axis
            const rx = seededRandom(i * 3 + 1) * 2 - 1; // -1 to 1
            const ry = seededRandom(i * 7 + 3) * 2 - 1; // -1 to 1

            // Scale by max offsets, with some minimum spread
            const x = rx * maxH * 0.8;
            const y = ry * maxV * 0.6;
    ...
    return (
        <>
            {planesData.current.map((plane, i) => {
                ...
                const aspect =
                    img && "width" in img && "height" in img
